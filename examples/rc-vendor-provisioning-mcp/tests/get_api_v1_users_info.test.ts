...
import { tool } from "../src/tools/get_api_v1_users_info.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_users_info", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_users_info");
    expect(tool.description).toBe("users.get()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/users.info";
