...
import { tool } from "../src/tools/get_api_v1_groups_members.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_groups_members", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_groups_members");
    expect(tool.description).toBe("groups.members()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/groups.members";
