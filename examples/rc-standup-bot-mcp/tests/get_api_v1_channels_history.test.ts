...
import { tool } from "../src/tools/get_api_v1_channels_history.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_channels_history", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_channels_history");
    expect(tool.description).toBe("channels.history()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/channels.history";
