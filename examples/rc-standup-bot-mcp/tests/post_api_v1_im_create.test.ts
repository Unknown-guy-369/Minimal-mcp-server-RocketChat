...
import { tool } from "../src/tools/post_api_v1_im_create.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: post_api_v1_im_create", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("post_api_v1_im_create");
    expect(tool.description).toBe("dm.new()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/dm.create";
