...
import { tool } from "../src/tools/get_api_v1_moderation_reports.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_moderation_reports", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_moderation_reports");
    expect(tool.description).toBe("moderation.reports()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/moderation.reports";
