...
import { tool } from "../src/tools/get_api_v1_sessions_list_all.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_sessions_list_all", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_sessions_list_all");
    expect(tool.description).toBe("sessions/list.all()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/sessions/list.all";
