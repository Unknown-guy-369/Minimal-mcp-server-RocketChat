...
import { tool } from "../src/tools/get_api_v1_livechat_department.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_livechat_department", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_livechat_department");
    expect(tool.description).toBe("livechat/department()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/livechat/department";
