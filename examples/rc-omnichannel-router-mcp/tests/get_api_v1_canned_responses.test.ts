...
import { tool } from "../src/tools/get_api_v1_canned_responses.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_canned_responses", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_canned_responses");
    expect(tool.description).toBe("canned-responses()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/canned-responses";
