...
import { tool } from "../src/tools/post_api_v1_channels_kick.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: post_api_v1_channels_kick", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("post_api_v1_channels_kick");
    expect(tool.description).toBe("channels.kick(rid:o, uid:o)");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/channels.kick";
