...
import { tool } from "../src/tools/post_api_v1_users_logoutOtherClients.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: post_api_v1_users_logoutOtherClients", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("post_api_v1_users_logoutOtherClients");
    expect(tool.description).toBe("users.logoutOtherClients()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/users.logoutOtherClients";
