...
import { tool } from "../src/tools/get_api_v1_channels_list.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_channels_list", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_channels_list");
    expect(tool.description).toBe("channels.ls()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/channels.list";
