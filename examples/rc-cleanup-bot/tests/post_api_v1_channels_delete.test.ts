...
import { tool } from "../src/tools/post_api_v1_channels_delete.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: post_api_v1_channels_delete", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("post_api_v1_channels_delete");
    expect(tool.description).toBe("channels.del([rid:o], [roomName:o])");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/channels.delete";
