...
import { tool } from "../src/tools/get_api_v1_chat_getMessage.js";
import { rcClient } from "../src/rc-client.js";

describe("Tool: get_api_v1_chat_getMessage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports correct name and description", () => {
    expect(tool.name).toBe("get_api_v1_chat_getMessage");
    expect(tool.description).toBe("chat.getMessage()");
  });
  ...
  it("calls rcClient.request with correct method, path, and payload", async () => {
    ...
    let expectedPath = "/api/v1/chat.getMessage";
